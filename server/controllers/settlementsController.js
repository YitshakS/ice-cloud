...
export async function loadSettlements(_, res) {
	...
	const json3 = JSON.parse(
		fs.readFileSync("json/שמות_יישובים_עם_קואורדינטות.geojson", "utf8")
	);
	// .catch((err) => console.log(err));
	// .catch(() => {
	...
	settlements = settlements.map((a) =>
		Object.assign(
			a,
			json3.features.find(
				(b) => parseInt(a.סמל_ישוב) === b.properties.SETL_CODE
			)
		)
	);
	...
	settlements = settlements.filter((o) =>
		Object.keys(o).includes("properties")
	);
	...
	settlements.forEach((settlement) => {
		...
		settlement.meteorological_station = closestLocation(
			// קואורדינטות הישוב
			{
				latitude: settlement.geometry.coordinates[1],
				longitude: settlement.geometry.coordinates[0],
			},
			// קואורדינטות התחנות
			stations
		);
